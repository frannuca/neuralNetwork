...
trait Weights extends Serializable{
  ...
  def randomizeWeigths{
    ...
    Ws.foreach(item => item match{
      case ((l1,l2),w)=> w.getArray().indices.foreach(i=> w.getArray()(i)= w.getArray()(i)*(1.0-(rngen.nextDouble())*0.25 ))
    })
  }

  generateLayerIndices.map(l =>{
    Ws += (l,l+1) -> (new Matrix[Double](layers(l).size+1,layers(l+1).size).random*0.1)
  })
